...
const InteractiveTable = () => {
    const [rows, setRows] = useState(() => {
        ...
        return savedRows ? JSON.parse(savedRows) : [
            { id: 1, company: "", status: "", industry: "", position: "", applicationLink: "", dueDate: "" }
        ];
    });
        ...
        <div className="interactive-table-container">
            <Form.Group>
                ...
                >
                    ...
                    <option value="On-site / Final Round">On-site / Final Round</option>
            ...
            <Table striped bordered hover>
                ...
                <tbody>
                    {rows.filter(row => row.status.includes(filter) || filter === '').map((row, index) => (
                        <tr key={row.id}>
                            <td>{index + 1}</td>
                            <td>
                                <TableAutoFill
