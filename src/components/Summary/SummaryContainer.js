...
function SummaryContainer() {
  ...
  const initializeDueCounts = () => {
    ...
    return savedDueCounts || { dueIn3Days: 0, dueIn5Days: 0, dueIn7Days: 0, dueCompany: []};
  };
  ...
  const updateDueCounts = () => {
    ...
    let dueIn3Days = 0, dueIn5Days = 0, dueIn7Days = 0, dueCompany = [];

    savedRows.forEach(row => {
        const dueDate = new Date(row.dueDate + 'T00:00');
        const daysUntilDue = Math.floor((dueDate - now) / (1000 * 3600 * 24));

        if(daysUntilDue >=0 && daysUntilDue < 2){
          dueCompany.push(row.company)
        }
        if (daysUntilDue <= 3) {
            dueIn3Days++;
        }
    ...
    const newDueCounts = { dueIn3Days, dueIn5Days, dueIn7Days, dueCompany};
    ...
    <Accordion defaultActiveKey="0">
      <Accordion.Item eventKey="0">
        ...
        <Accordion.Body>
          ...
          <DueDateSummary counts={dueCounts} companies={dueCounts.dueCompany} />
